package com.bankhub.investment.infrastructure.web.exception;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Captura exceções de negócio (Saldo insuficiente detectado no Account Service, Ticker inválido, etc).
     */
    ...
    /**
     * Fallback (Catch-All) para erros inesperados (ex: falha no banco MongoDB da corretora).
     * Garante que a stacktrace NUNCA vaze para a rede.
     */
    @ExceptionHandler(Exception.class)
    public ProblemDetail handleGenericException(Exception ex) {
        log.error("Erro interno do servidor não tratado explicitamente no Motor de Investimentos: ", ex);

        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(HttpStatus.INTERNAL_SERVER_ERROR, "Ocorreu um erro interno no Home Broker. Tente novamente mais tarde.");
